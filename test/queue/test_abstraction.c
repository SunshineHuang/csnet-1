...
void abstraction_test_increment(void)
{
	...
	LFDS_ALIGN(LFDS_ALIGN_SINGLE_POINTER) volatile unsigned long long int shared_counter, atomic_shared_counter;
...
thread_return_t CALLING_CONVENTION abstraction_test_internal_thread_increment(void *shared_counter)
{
	...
	abstraction_test_helper_increment_non_atomic(shared_counter);
...
thread_return_t CALLING_CONVENTION abstraction_test_internal_thread_atomic_increment(void *shared_counter)
{
	...
	abstraction_test_helper_increment_atomic(shared_counter);
...
void abstraction_test_cas(void)
{
	unsigned int loop, cpu_count;
	thread_state_t *thread_handles;
	struct abstraction_test_cas_state *atcs;
	LFDS_ALIGN(LFDS_ALIGN_SINGLE_POINTER) volatile unsigned long long int shared_counter;
	unsigned long long int local_total = 0;

	// TRD : number_logical_processors can be any value in its range

	/* TRD : here we test abstraction_cas

					 we run one thread per CPU
					 we use abstraction_cas() to increment a shared counter
...
thread_return_t CALLING_CONVENTION abstraction_test_internal_thread_cas(void *abstraction_test_cas_state)
{
	...
	abstraction_test_helper_cas(atcs->shared_counter, &atcs->local_counter);
...
void abstraction_test_dcas(void)
{
	unsigned int loop, cpu_count;
	thread_state_t *thread_handles;
	struct abstraction_test_dcas_state *atds;
	LFDS_ALIGN(LFDS_ALIGN_DOUBLE_POINTER) volatile unsigned long long int shared_counter[2] = { 0, 0 };
	unsigned long long int local_total = 0;
...
thread_return_t CALLING_CONVENTION abstraction_test_internal_thread_dcas(void *abstraction_test_dcas_state)
{
	...
	abstraction_test_helper_dcas(atds->shared_counter, &atds->local_counter);
